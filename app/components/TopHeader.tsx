            ...
            <div className={styles.mobileHeader}>
                <div className={styles.mobileTopRow}>
                    <div className={styles.mobileLeftGroup}>
                        ...
                        <div className={styles.mobileBranding}>
                            <Link href="/" className={styles.mobileLogoLink}>
                                <span className={styles.mobileLogo}>Izy</span>
                                <span className={styles.mobileLogoAccent}>Health</span>
                            </Link>
                            <span className={styles.mobileLabChip}>{labName}</span>
                        </div>
                    </div>
                    <div className={styles.mobileRightGroup}>
                        ...
                        >
                            <i className={`fa ${showMobileSearch ? 'fa-xmark' : 'fa-magnifying-glass'}`}></i>
                        </div>
        ...
        {showMobileSearch && (
            <div className={styles.mobileSearchOverlay} ref={searchRef}>
                <div className={styles.mobileSearchBar}>
                    <i className={`fa ${searching ? 'fa-spinner fa-spin' : 'fa-magnifying-glass'}`}></i>
                    <input 
                        type="text" 
                        placeholder="Search by Bill or Report ID..." 
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        onFocus={() => searchQuery.trim() && setShowSearchResults(true)}
                        autoFocus
                    />
                    <button className={styles.mobileSearchClose} onClick={() => { setShowMobileSearch(false); setSearchQuery(''); setShowSearchResults(false); }}>
                            ...
                            <>
                                {searchResults.bills.length > 0 && (
                                    <div className={styles.resultSection}>
                                        <div className={styles.sectionHeader}>Bills</div>
                                        {searchResults.bills.map((bill) => (
                                            <div key={bill.id} className={styles.resultItem} onClick={() => { handleResultClick(bill.url); setShowMobileSearch(false); }}>
                                                <div className={`${styles.resultIcon} ${styles.billIcon}`}><i className="fa fa-file-invoice-dollar"></i></div>
                                                <div className={styles.resultInfo}>
                                                    <span className={styles.resultTitle}>{bill.patientName}</span>
                                                    <span className={styles.resultMeta}>Bill ID: <span className={styles.shortId}>#{bill.shortId}</span></span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                {searchResults.reports.length > 0 && (
                                    <div className={styles.resultSection}>
                                        <div className={styles.sectionHeader}>Reports</div>
                                        {searchResults.reports.map((report) => (
                                            <div key={report.id} className={styles.resultItem} onClick={() => { handleResultClick(report.url); setShowMobileSearch(false); }}>
                                                <div className={`${styles.resultIcon} ${styles.reportIcon}`}><i className="fa fa-file-medical"></i></div>
                                                <div className={styles.resultInfo}>
                                                    <span className={styles.resultTitle}>{report.patientName}</span>
                                                    <span className={styles.resultMeta}>Report ID: <span className={styles.shortId}>#{report.shortId}</span></span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
